...
**Produces:** `Parent` component `{ entity: Entity | null }`; `setParent(world, child, parent)` (cycle-reject), `childrenOf`, `ancestorsOf`, `rootsOf` (cached child-index resource, rebuilt from Parent change signals); `despawnTree(world, entity)`; orphan policy (despawned parent → children reparent to null) via plugin cleanup system; `composeTransforms(Local, World, composeFn)` plugin factory walking roots→leaves per tick.

**Acceptance:** cycle rejection, orphan policy, parent-before-child compose order, snapshot round-trip with hierarchy; package published/linked; engine tests green.
